...
export default function AddMedicationScreen({ navigation }) {
  ...
  const [reminderTimes, setReminderTimes] = useState([new Date()]); // Array of Date objects
  ...
  return (
    ...
    >
      <View style={[styles.header, rtl.row, { backgroundColor: theme.colors.surface }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.text }]}>{t('Add Medication')}</Text>
        <View style={{ width: 28 }} />
      </View>
