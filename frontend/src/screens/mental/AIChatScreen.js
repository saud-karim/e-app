...
export default function AIChatScreen({ navigation }) {
  ...
  const [messages, setMessages] = useState([
    { id: '1', role: 'assistant', content: t("Hello! I am your safe space AI assistant. I'm here to listen, support, and talk with you. How are you feeling today?") }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error(error);
      const errorMessage = { id: (Date.now() + 1).toString(), role: 'assistant', content: t("Sorry, I'm having trouble connecting right now.") };
  ...
  const renderMessage = ({ item }) => {
    ...
    return (
      <View style={[styles.messageBubble, isUser ? [styles.userBubble, { backgroundColor: theme.colors.primary }] : [styles.aiBubble, { backgroundColor: theme.colors.surface }]]}>
        ...
        <Text style={[styles.messageText, { color: isUser ? '#FFF' : theme.colors.text, textAlign: rtl.isRTL ? 'right' : 'left' }]}>
          {t(item.content)}
        </Text>
      </View>
    );
